...
// Test that we can change the secrets provider for a stack even if we've lost the original secrets key, this
// sets up a mock stack using a passphrase secret provider and then changes it to a new passphrase provider, with
// existing secrets in the state and config which should be discarded.
//
//nolint:paralleltest // mutates global state
func TestChangeSecretsProvider_DiscardWithSecrets(t *testing.T) {
	ctx := context.Background()
	...
	// Ideally this would be injected but the cmd functions repeatedly access global state to get the current
	// backend.
	passphraseState, secretsManager, err := passphrase.NewPassphraseSecretsManager("testing")
	require.NoError(t, err)
	originalPassphraseState := secretsManager.State()
	snapshot := &deploy.Snapshot{
		SecretsManager: secretsManager,
		Resources: []*resource.State{
			{
...
`), 0o600)
	...
	// Write a dummy config file with a secret in it
	passphraseEncrypter, err := secretsManager.Encrypter()
	require.NoError(t, err)
	secretBar, err := passphraseEncrypter.EncryptValue(ctx, "bar")
	require.NoError(t, err)
	cfgKey := config.MustMakeKey("testStack", "secret")
	cfg := workspace.ProjectStack{
		EncryptionSalt: passphraseState,
		Config: config.Map{
			cfgKey: config.NewSecureValue(secretBar),
		},
	}
	...
	// Check that the snapshot is still a passphrase secrets manager
	assert.Equal(t, "passphrase", snapshot.SecretsManager.Type())
	// But with a new state
	assert.NotEqual(t, originalPassphraseState, snapshot.SecretsManager.State())
	// Check that the snapshot has removed the secret value
	_, ok := snapshot.Resources[0].Outputs["foo"]
	assert.False(t, ok)
	// Check the config has removed the secret value
	project, err := workspace.LoadProject("Pulumi.yaml")
	require.NoError(t, err)
	projectStack, err := workspace.LoadProjectStack(project, "Pulumi.testStack.yaml")
	require.NoError(t, err)
	_, ok = projectStack.Config[cfgKey]
	require.False(t, ok)
}
